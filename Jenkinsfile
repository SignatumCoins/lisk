...
def teardown(test_name) {
	...
	try {
		nvm(getNodejsVersion()) {
			...
			./node_modules/.bin/nyc report --report/dir ./framework/test/mocha/.coverage-unit/
			cp test/.coverage-unit/lcov.info coverage_${test_name}/ || true
			./node_modules/.bin/nyc report --reporter=cobertura --report/dir ./framework/test/mocha/.coverage-unit/
